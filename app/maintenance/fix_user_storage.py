...
async def fix_datamining_service():
    ...
    datamining_path = os.path.join('app', 'services', 'analytics', 'datamining_service.py')
...
async def _fix_user_storage(verbose=False):
    """
    Kullanıcı depolama sorunlarını düzeltir ve test kullanıcıları ekler.
    ...
    return users_fixed and (users_added is not False) and dm_fixed

def fix_user_storage(verbose=False):
    """
    Kullanıcı depolama sorunlarını düzelten senkron wrapper fonksiyon.
    ...
    Returns:
        bool: İşlem başarılı ise True
    """
    return asyncio.run(_fix_user_storage(verbose))

async def main():
    """Ana fonksiyon"""
    await _fix_user_storage(verbose=True)
