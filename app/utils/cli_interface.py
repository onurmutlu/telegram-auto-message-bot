...
# Yol: /Users/siyahkare/code/telegram-bot/app/utils/cli_interface.py
...
def print_banner():
    """Banner gösterir"""
    console.print(Panel("""
[bold cyan]Telegram Bot Sistemi v3.8.0[/bold cyan]
[blue]© 2025 SiyahKare Yazılım - Tüm Hakları Saklıdır[/blue]
    """, title="Hoş Geldiniz", expand=False))
...
async def handle_keyboard_input(console, service_wrapper, stop_event):
    ...
    while not stop_event.is_set():
        try:
            ...
            # Komutu işle
            should_exit = await process_command(command, service_wrapper)
...
async def process_command(cmd, service_wrapper):
    """
    ...
    try:
        ...
        if cmd == 'q':
            ...
            await show_status(service_wrapper)
            
        elif cmd == 'i':
            await show_statistics(service_wrapper)
            ...
            await restart_services(service_wrapper)
        ...
        # YENİ: Demografik analiz komutu
            ...
            await display_demographics(service_wrapper)
            
        # YENİ: Veri madenciliği menüsü
        elif cmd == 'm':
            await show_mining_menu(service_wrapper)
            
        # YENİ: Hedefli kampanya menüsü  
        elif cmd == 't':
            await show_campaign_menu(service_wrapper)
        
        # Grup analitik raporu gösterme komutu
        elif cmd == 'a':
            await display_analytics(service_wrapper)
        
        # Hata izleme raporu gösterme komutu
        elif cmd == 'e':
            await display_error_report(service_wrapper)
...
async def display_demographics(service_wrapper):
    """Toplanan demografik verileri gösteren bir özet rapor oluşturur"""
    try:
        data_mining = service_wrapper.get_service("datamining")
...
async def show_mining_menu(service_wrapper):
...
async def show_campaign_menu(service_wrapper):
    ...
    # Servisleri al
    data_mining = service_wrapper.get_service("datamining")
    invite_service = service_wrapper.get_service("invite")
    ...
    from app.utils.targeted_campaign import TargetedCampaign
...
async def display_analytics(service_wrapper):
    """Grup analitik raporunu gösterir"""
    try:
        analytics_service = service_wrapper.get_service("analytics")
...
async def display_error_report(service_wrapper):
    """Hata izleme raporunu gösterir"""
    try:
        error_service = service_wrapper.get_service("error")
...
async def view_log_file():
    ...
    if not os.path.exists(log_path):
        console.print(f"[red]Log dosyası bulunamadı: {log_path}[/red]")
        return
    ...
    try:
        ...
        console.print(Panel("\n".join(last_lines), title="Son Loglar", expand=False))
    except Exception as e:
        console.print(f"[red]Log dosyası okuma hatası: {str(e)}[/red]")

async def service_picker(service_wrapper):
    """Servis seçme menüsünü gösterir"""
    try:
        # Mevcut servisleri al
        active_services = service_wrapper.get_active_services()
        available_services = service_wrapper.get_available_services()
        
        # Tablo oluştur
        table = Table(title="Servisler")
        ...
        for i, service in enumerate(available_services):
            ...
            status = "[green]Aktif[/green]" if active else "[red]Devre Dışı[/red]"
            table.add_row(str(i+1), service, status)
            
        console.print(table)
        
        # Kullanıcıdan seçim iste
        console.print("[yellow]Servis numarası seçin (0: İptal):[/yellow]", end=" ")
        ...
        try:
            ...
            if 0 <= index < len(available_services):
                service_name = available_services[index]
                ...
                if is_active:
                    # Devre dışı bırak
                    console.print(f"[yellow]'{service_name}' servisi devre dışı bırakılıyor...[/yellow]")
                    success = await service_wrapper.remove_service(service_name)
                    if success:
                        console.print(f"[green]'{service_name}' servisi devre dışı bırakıldı.[/green]")
                    else:
                        console.print(f"[red]'{service_name}' servisi devre dışı bırakılamadı![/red]")
                else:
                    # Aktifleştir
                    console.print(f"[yellow]'{service_name}' servisi aktifleştiriliyor...[/yellow]")
                    success = await service_wrapper.add_service(service_name)
                    if success:
                        console.print(f"[green]'{service_name}' servisi aktifleştirildi.[/green]")
                    else:
                        console.print(f"[red]'{service_name}' servisi aktifleştirilemedi![/red]")
            else:
                console.print("[red]Geçersiz seçim![/red]")
        except ValueError:
            console.print("[red]Geçersiz giriş! Bir sayı girmelisiniz.[/red]")
