...
# from app.services.event_service import Event, on_event
...
class ErrorService(BaseService):
    ...
    def __init__(self, client=None, config=None, db=None, stop_event=None):
        ...
        try:
            # Eğer get_db_pool fonksiyonu mevcutsa
            # from database.db_connection import get_db_pool
            # self.db_pool = get_db_pool()
            self.db_pool = None  # Şimdilik devre dışı
        except (ImportError, AttributeError):
            self.db_pool = None
            logger.warning("Veritabanı bağlantı havuzu oluşturulamadı, bazı özellikler çalışmayabilir")
    ...
    async def _emit_error_event(self, error: ErrorRecord):
        ...
        try:
            ...
            # Event bus'ı al - şimdilik devre dışı bırakıldı
            # from app.services.event_service import EventBus
            # event_bus = EventBus()
            
            # Event'i tetikle - şimdilik devre dışı bırakıldı
            ...
            #         source="error_service"
            #     )
            logger.warning(f"Hata olayı: {error.severity} - {error.error_type}: {error.message}")
    ...
    async def _emit_error_alert(self, source: str, category: str = "GENERAL"):
        ...
        # Event bus'ı şimdilik devre dışı bırakıldı
        # from app.services.event_service import EventBus
        # event_bus = EventBus()
        # await event_bus.emit(
